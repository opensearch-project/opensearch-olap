 ...
 *
 * <p>The scheduler converts plan fragments into stages, creates tasks for each stage
 * based on shard routing, and dispatches tasks to target data nodes.
 *
 * <p>Execution flow:
 * <ol>
 *   <li>Receive fragmented plan from VeloxPlanConverter + PlanFragmenter</li>
 *   <li>Build stages and create tasks via ShardRouter</li>
 *   <li>Dispatch leaf-stage tasks to data nodes (via TransportService)</li>
 *   <li>When leaf tasks complete, dispatch dependent stages</li>
 *   <li>Collect final results from root stage</li>
 * </ol>
 *
 * <p>Inspired by Presto's SqlQueryScheduler which manages stage lifecycle and
 * coordinates task creation across nodes. Key differences:
 * <ul>
 *   <li>Uses OpenSearch TransportService instead of HTTP for task dispatch</li>
 *   <li>Shard routing is derived from OpenSearch ClusterState</li>
 *   <li>Tasks feed Lucene data via ExternalStream (no Hive splits)</li>
 * </ul>
 */
public class QueryScheduler {
     ...
     */
    public QueryExecution schedule(
    ...
    /**
     * Called when a task completes on a data node.
     * Advances the query execution by dispatching ready stages.
     */
    public void onTaskCompleted(QueryId queryId, TaskId taskId) {
        ...
        if (execution.getStages().stream()
            .allMatch(s -> s.getState() == Stage.StageState.FINISHED)) {
        ...
        if (props.getDistribution() == FragmentProperties.Distribution.SOURCE) {
            ...
            for (Map.Entry<DiscoveryNode, List<ShardId>> entry : routing.entrySet()) {
                ...
                TaskDescriptor task = new TaskDescriptor(
                    taskId, fragment, entry.getKey(), entry.getValue()
                );
                stage.addTask(task);
            ...
            TaskDescriptor task = new TaskDescriptor(
                taskId, fragment, localNode, List.of()
            );
            stage.addTask(task);
            execution.getTaskTracker().register(task);
        }
    ...
    private void dispatchStage(QueryExecution execution, Stage stage) {
        ...
        logger.info("Dispatching stage {} with {} tasks",
    ...
    /**
     * Dispatch a task to its target node via TransportService.
     * This method is called from the scheduler and sends ExecuteFragmentRequest
     * to the target data node.
     *
     * The actual transport dispatch is handled by NodeResultCollector which
     * holds a reference to TransportService.
     */
    private void dispatchTask(QueryId queryId, TaskDescriptor task) {
        ...
        logger.debug("Dispatching task {} to node {}",
            task.getTaskId(), task.getTargetNode().getName());
    }
}
