 ...
 *
 * <p>Uses the cluster routing table to determine which nodes own which shards
 * for a given index. Groups shards by their owning node so that each task
 * processes all local shards on a single node.
 */
public class ShardRouter {
     ...
     */
    public Map<DiscoveryNode, List<ShardId>> routeShards(ClusterState clusterState, String indexName) {
        ...
        for (int shardNum = 0; shardNum < indexRouting.shards().size(); shardNum++) {
            ...
            nodeToShards.computeIfAbsent(node, k -> new ArrayList<>())
                .add(primaryShard.shardId());
        }
