 ...
 *
 * <p>A stage corresponds to a PlanFragment and consists of one or more tasks, each running on a
 * different data node. The stage tracks the collective state of all its tasks.
 *
 * <p>Modeled after Presto's SqlStageExecution: a stage is the unit of scheduling, while tasks are
 * the unit of execution on individual nodes.
 */
public class Stage {
  ...
  public Stage(StageId stageId, PlanFragment fragment, List<StageId> upstreamStageIds) {
    ...
    this.upstreamStageIds =
        upstreamStageIds != null
            ? Collections.unmodifiableList(upstreamStageIds)
  ...
  private boolean isValidTransition(StageState from, StageState to) {
    switch (from) {
      ...
      case SCHEDULING:
        return to == StageState.RUNNING || to == StageState.FAILED || to == StageState.CANCELLED;
      case RUNNING:
        return to == StageState.FINISHED || to == StageState.FAILED || to == StageState.CANCELLED;
      default:
        return false;
    }
  }
