 ...
 *
 * <p>A QueryExecution is created by the QueryScheduler for each incoming query.
 * It holds the plan fragments, stages, and overall execution state.
 *
 * <p>Modeled after Presto's SqlQueryExecution: the coordinator creates stages
 * from plan fragments and orchestrates their execution.
 */
public class QueryExecution {
    ...
    /**
     * Create stages from plan fragments.
     * Each fragment becomes a stage identified by StageId(queryId, fragmentId).
     */
    ...
    public List<Stage> getReadyStages() {
        ...
        for (Stage stage : stagesByFragmentId.values()) {
            ...
            boolean allUpstreamDone = stage.getUpstreamStageIds().stream()
                .allMatch(upId -> {
                    Stage upstream = stagesByFragmentId.get(upId.getStageNumber());
                    return upstream != null
                        && upstream.getState() == Stage.StageState.FINISHED;
                });
            if (allUpstreamDone) {
                ready.add(stage);
            }
