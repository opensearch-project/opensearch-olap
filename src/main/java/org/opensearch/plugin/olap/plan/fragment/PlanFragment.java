 ...
 *
 * <p>Each fragment contains a sub-tree of PlanNodes that can be executed as a unit
 * on a single node. Fragments are connected by exchange boundaries where data flows
 * from one fragment's output to another fragment's input (via ExternalStream).
 *
 * <p>For a simple SELECT ... FROM ... WHERE ... query on a single table:
 * <ul>
 *   <li>Fragment 0 (leaf): TableScan + Filter + partial Aggregation → runs on data nodes</li>
 *   <li>Fragment 1 (root): final Aggregation + Project → runs on coordinator</li>
 * </ul>
 */
