...
public class VeloxExprConverter {
    ...
    private TypedExpr convertCall(RexCall call) {
        ...
        if (kind == SqlKind.IS_NOT_NULL) {
            ...
            TypedExpr isNull = new CallTypedExpr(
                new BooleanType(),
    ...
    private TypedExpr convertIn(RexCall call) {
        ...
        for (int i = 1; i < operands.size(); i++) {
            ...
            TypedExpr eq = new CallTypedExpr(
                new BooleanType(),
                List.of(lhs, rhs),
                "eq"
            );
    ...
    private TypedExpr convertBetween(RexCall call) {
        ...
        TypedExpr gte = new CallTypedExpr(
            new BooleanType(),
            List.of(value, lower),
            "gte"
        );
        TypedExpr lte = new CallTypedExpr(
            new BooleanType(),
            List.of(value, upper),
            "lte"
        );
