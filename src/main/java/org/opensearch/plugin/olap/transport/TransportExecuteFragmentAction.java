 ...
 *
 * <p>This is the core data-node handler. When the coordinator dispatches a fragment
 * to this node, the handler:
 * <ol>
 *   <li>Deserializes the Velox plan from JSON</li>
 *   <li>Acquires Lucene searchers for the assigned shards</li>
 *   <li>Reads doc values into Arrow batches via LuceneArrowReader</li>
 *   <li>Feeds Arrow batches into velox4j ExternalStream</li>
 *   <li>Executes the Velox plan via velox4j QueryExecutor</li>
 *   <li>Serializes results and sends response back to coordinator</li>
 * </ol>
 *
 * <p>Inspired by Presto's TaskResource/SqlTaskExecution which receives plan fragments
 * and creates Velox tasks. Key difference: we read data from Lucene instead of
 * Hive splits, feeding it through ExternalStream.
 */
public class TransportExecuteFragmentAction
    ...
    public TransportExecuteFragmentAction(
        ...
        super(ExecuteFragmentAction.NAME, transportService, actionFilters,
            ExecuteFragmentRequest::new);
    ...
    protected void doExecute(
        ...
        ActionListener<ExecuteFragmentResponse> listener
    ) {
        ...
        threadPool.executor(ThreadPool.Names.SEARCH).execute(() -> {
            try {
                ...
                logger.error("Failed to execute fragment {} for query {}",
                    request.getFragmentId(), request.getQueryId(), e);
    ...
    private ExecuteFragmentResponse executeFragment(ExecuteFragmentRequest request) {
        ...
        logger.info("Executing fragment {} for query {} on {} shards",
            request.getFragmentId(), queryId, shardIds.size());
        ...
        ExternalStreamBridge bridge = new ExternalStreamBridge(
            veloxLifecycle.getSession()
        );
        ...
        try {
            ...
            byte[] resultData = executor.execute(
