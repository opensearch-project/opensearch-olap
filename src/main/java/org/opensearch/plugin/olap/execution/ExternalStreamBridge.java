 ...
 *
 * <p>This is the key integration point where Java-side Lucene data enters the
 * Velox C++ execution engine. The flow:
 * <ol>
 *   <li>LuceneArrowReader produces Arrow VectorSchemaRoot batches</li>
 *   <li>ExternalStreamBridge converts Arrow → Velox RowVector via Arrow C Data Interface</li>
 *   <li>RowVectors are pushed into a velox4j BlockingQueue</li>
 *   <li>Velox's TableScanNode reads from the BlockingQueue via ExternalStreamTableHandle</li>
 * </ol>
 *
 * <p>The BlockingQueue provides back-pressure: if Velox consumes slower than
 * Lucene produces, the put() call will block until space is available.
 */
public class ExternalStreamBridge implements AutoCloseable {
     ...
     *
     * <p>Converts the Arrow VectorSchemaRoot to a Velox RowVector using the
     * Arrow C Data Interface (zero-copy when possible), then pushes it into
     * the native blocking queue.
     */
     ...
     * This unblocks the Velox TableScanNode which will return end-of-data.
     */
    public void noMoreInput() {
        if (queue != null) {
            queue.noMoreInput();
            logger.debug("ExternalStreamBridge: noMoreInput signaled, total rows={}",
                rowCount.get());
        }
    }
