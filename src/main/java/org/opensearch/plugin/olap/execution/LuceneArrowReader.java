 ...
 *
 * <p>This is the critical bridge between OpenSearch's row-oriented Lucene storage
 * and the columnar Arrow format needed by Velox. For each shard:
 * <ol>
 *   <li>Acquire a Lucene searcher via IndexShard.acquireSearcher()</li>
 *   <li>Iterate over leaf readers (segments)</li>
 *   <li>Read doc values column-by-column into Arrow vectors</li>
 *   <li>Feed the Arrow batches into ExternalStreamBridge</li>
 * </ol>
 *
 * <p>Uses doc values (not stored fields or source) because:
 * <ul>
 *   <li>Doc values are columnar on disk, matching Arrow's columnar layout</li>
 *   <li>Doc values support sequential iteration (no random seeks)</li>
 *   <li>Doc values are always forward-only per segment, ideal for scan operations</li>
 * </ul>
 */
public class LuceneArrowReader {
    ...
    public void readShardIntoStream(
        ...
        try (Engine.Searcher searcher = shard.acquireSearcher("olap-velox")) {
            ...
            ArrowBatchBuilder batchBuilder = new ArrowBatchBuilder(
                allocator, columnSpecs, BATCH_SIZE
            );
            ...
            for (LeafReaderContext leafCtx : searcher.getIndexReader().leaves()) {
                ...
                for (int startDoc = 0; startDoc < maxDoc; startDoc += BATCH_SIZE) {
                    ...
                    VectorSchemaRoot batch = batchBuilder.buildBatch(
                        leafCtx.reader(), startDoc, endDoc
                    );

                    bridge.feedBatch(batch);

                    logger.trace("Fed batch [{}-{}) from shard {} segment {}",
                        startDoc, endDoc, shardId, leafCtx.ord);
                }
            }

            logger.debug("Finished reading shard {}: segments={}",
                shardId, searcher.getIndexReader().leaves().size());
        }
    }

    /**
     * Resolve column specifications from index mappings.
     * Maps OpenSearch field types to Arrow types and DocValue types.
     */
    private List<ColumnSpec> resolveColumnSpecs(
        ...
        for (String fieldName : fieldNames) {
            ...
            if (arrowType != null && dvType != null) {
                ...
                logger.warn("Unsupported field type {} for field {}, skipping",
