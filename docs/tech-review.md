...
- Window functions: eventstats (COUNT, SUM, AVG, MIN, MAX with PARTITION BY) + ranking family (ROW_NUMBER, RANK, DENSE_RANK) via Velox WindowNode. ProjectToWindowRule decomposes RexOver → LogicalWindow → PhysicalWindow → WindowNode. Ranking functions are narrowed to Velox's INTEGER registration (Spark dialect overwrites Presto's BIGINT at init time) and cast back to BIGINT via an inserted ProjectNode so downstream consumers see Calcite's declared type. PPL `dedup` compiles to `row_number() <= 1` and now executes through Velox. NTILE/NTH_VALUE deferred (offset-type mismatch with Spark INTEGER registration needs argument coercion).
...
- ClickBench benchmark: 43 PPL queries migrated from the SQL plugin's `PPLClickBenchIT`. **All 43 queries run end-to-end through Velox.** Uncovered during this migration:
  - **`!=` comparisons** lowered to Velox's `notequalto`, which is only registered for DECIMAL in the Spark dialect — rewritten to `not(equalto(a, b))` with binary-type coercion so `SMALLINT != INTEGER_literal` no longer crashes in Velox signature resolution. See `VeloxExprConverter.convertCall`'s NOT_EQUALS branch.
  - **`AGG_ARGS` hint cluster-copy bug**: the SQL plugin attaches `AGG_ARGS` hints to `LogicalAggregate` (for `bucket_nullable=false` / nested-agg bookkeeping), registered on its own `RelOptCluster.HintStrategyTable`. `ClusterCopyShuttle` creates a fresh cluster whose strategy table is empty, and Calcite's `HintStrategyTable.canApply` asserts `"hint AGG_ARGS must be present"` as soon as any rule inspects hints. The hints' semantic effects (isNotNull filter, etc.) are already materialized upstream before we copy, so stripping the hint list in `ClusterCopyShuttle.visit(LogicalAggregate)` is safe and unblocks 29 ClickBench queries. See `PhysicalOptimizer.ClusterCopyShuttle`.
  - **PPL `extract(part FROM datetime)`**: lowered by the SQL plugin to its own EXTRACT UDF (SqlKind=OTHER_FUNCTION, operator name "EXTRACT"), not Calcite's built-in SqlStdOperatorTable.EXTRACT. Velox has no generic `extract(VARCHAR, TIMESTAMP)` — but it does register dedicated date-part scalars (`year`, `month`, `week_of_year`, `day`, `day_of_week`, `day_of_year`, `hour`, `minute`, `second`). Both Presto (BIGINT) and Spark (INTEGER) variants coexist under the same name, so the converter requests Calcite's declared BIGINT and lets Velox's resolver pick the matching physical signature — no cast needed. See `VeloxExprConverter.convertExtract`.
  - **PPL `span(field, count, unit)`**: Velox has no `span` scalar, so `VeloxExprConverter.convertSpan` lowers it in four shapes — **TIMESTAMP count=1** → `date_trunc`; **TIMESTAMP count>1** on fixed-length second-aligned units (s/m/h/d) → `from_unixtime(floor(to_unixtime(ts)/N)*N)` with `N = count × unit_seconds`; **DATE count=1** on d/w/M/q/y → `date_trunc` (Velox's DATE variant); **integer numeric** → Euclidean-modulo `col - ((col mod w + w) mod w)` in integer space (preserves BIGINT precision above 2^53 AND matches `floor` semantics for negative values — the naive integer `floor(col/w)*w` mis-buckets negatives because Velox's integer divide truncates toward zero and integer floor is a no-op); **floating-point numeric** → `floor(col/w)*w` in DOUBLE (where `floor` is a real floor). Fractional DECIMAL widths accepted on DOUBLE/REAL fields via the DOUBLE path; rejected on integer fields to preserve BIGINT precision. Temporal kind is resolved via Calcite's `SqlTypeName` plus PPL's `ExprUDT` tag (DATE/TIME UDTs report VARCHAR with a UDT annotation), so `span(date_col, 1d)` doesn't get misrouted to the numeric path. Falls back for: **TIME** columns (velox4j has no TimeType wrapper, so `date_trunc(TIME)` cannot be emitted); DATE sub-day units; DATE count>1; TIMESTAMP `ms`/`us`; month/quarter/year with count>1; **week with count>1** (Unix epoch is a Thursday but `date_trunc('week')` anchors to Monday); DECIMAL columns. Unblocks q43 (per-minute bucketing) and makes PPL's canonical time/numeric bucketing primitive vectorizable.
...
| Priority | Item | Gap vs RFC | Current State |
|----------|------|-----------|---------------|
| ~~**Medium**~~ | ~~Window functions~~ | ~~ROW_NUMBER, RANK, LAG, LEAD, etc.~~ | **Done** — eventstats (COUNT/SUM/AVG/MIN/MAX) + ranking family (ROW_NUMBER, RANK, DENSE_RANK) via PhysicalWindow → Velox WindowNode. PPL `dedup` compiles to `row_number() <= 1` and executes through Velox. NTILE/NTH_VALUE/LAG/LEAD/FIRST_VALUE/LAST_VALUE deferred (NTILE and NTH_VALUE blocked on offset-argument coercion; others not yet whitelisted in `canVectorize`). |
