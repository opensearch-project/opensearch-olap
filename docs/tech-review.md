...
- ClickBench benchmark: 43 PPL queries migrated from the SQL plugin's `PPLClickBenchIT`. **42 queries run end-to-end through Velox**; only q43 (`span(TIMESTAMP, 1m)`) is `@Ignore`'d — a specific unregistered Velox UDF. Uncovered during this migration:
